...
history_df = ratios_df[ratios_df["year"] != "TTM"].copy()

history_df = history_df.sort_values(["company_id", "year_num"])

# ---------------------------------------
# Latest Annual Record
# ---------------------------------------

latest_df = history_df.groupby("company_id").tail(1)
...
balance_df["year_num"] = (
    balance_df["year"].astype(str).str.extract(r"(\d{4})")[0].astype(float)
)

balance_df = balance_df.sort_values(["company_id", "year_num"])

# ---------------------------------------
# Merge Latest Data
# ---------------------------------------

latest_df = latest_df.merge(market_df, on=["company_id", "year"], how="left")

latest_df = latest_df.merge(companies_df, on="company_id", how="left")

latest_df = latest_df.merge(sector_df, on="company_id", how="left")
...
latest_df.to_csv(os.path.join(OUTPUT_DIR, "latest_company_data.csv"), index=False)
...
for company in latest_df["company_id"]:
    ...
    hist = (
        history_df[history_df["company_id"] == company].sort_values("year_num").tail(3)
    )
...
for company in latest_df["company_id"]:
    ...
    bs = balance_df[balance_df["company_id"] == company].sort_values("year_num").tail(3)
...
final_df = final_df.sort_values(["company_id", "type", "rule_id"])
