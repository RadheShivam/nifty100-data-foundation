...
annual_ratios = ratios_df[~ratios_df["year"].astype(str).str.upper().eq("TTM")].copy()

annual_ratios["year_num"] = (
    annual_ratios["year"].astype(str).str.extract(r"(\d{4})")[0].astype(float)
)

latest_ratios = annual_ratios.sort_values("year_num").groupby("company_id").tail(1)
...
required_columns = ["company_id", "company_name", "broad_sector"] + FEATURES
...
elbow_path = os.path.join(REPORT_DIR, "elbow_plot.png")
...
data["distance_to_centroid"] = [
    distances[i][cluster] for i, cluster in enumerate(cluster_labels)
]
...
cluster_summary = data["cluster"].value_counts().sort_index()
...
profile_csv = os.path.join(OUTPUT_DIR, "cluster_profile.csv")
...
for cluster in sorted(data["cluster"].unique()):
    ...
    companies = (
        data[data["cluster"] == cluster].sort_values("distance_to_centroid").head(5)
    )

    print(companies[["company_name", "broad_sector", "distance_to_centroid"]])
