...
ratios_df["year_num"] = ratios_df["year"].astype(str).str.extract(r"(\d{4})")[0]

ratios_df["year_num"] = pd.to_numeric(ratios_df["year_num"], errors="coerce")
...
companies_df = companies_df[["id", "company_name"]]

sector_df = sector_df[["company_id", "broad_sector", "sub_sector"]].drop_duplicates(
    subset="company_id"
)
...
valuation_df.rename(columns={"year_x": "year"}, inplace=True)

if "year_y" in valuation_df.columns:
    valuation_df.drop(columns=["year_y"], inplace=True)

valuation_df = valuation_df.merge(sector_df, on="company_id", how="left")

valuation_df = valuation_df.merge(
    companies_df, left_on="company_id", right_on="id", how="left"
)
...
valuation_df = valuation_df[~valuation_df["year"].isin(["TTM"])].copy()

valuation_df = valuation_df[
    ~valuation_df["year"].str.contains("9m|15", case=False, na=False)
]

valuation_df["year_num"] = pd.to_numeric(
    valuation_df["year"].str.extract(r"(\d{4})")[0], errors="coerce"
)

valuation_df = (
    valuation_df.sort_values("year_num").groupby("company_id", as_index=False).tail(1)
)
...
valuation_df["fcf_yield_pct"] = (
    valuation_df["free_cash_flow_cr"] / valuation_df["market_cap_crore"]
) * 100
...
print(
    valuation_df[
        ["company_id", "free_cash_flow_cr", "market_cap_crore", "fcf_yield_pct"]
    ].head(10)
)
...
valuation_df = valuation_df.merge(sector_pe, on="broad_sector", how="left")
...
valuation_df["pe_vs_sector_median_pct"] = valuation_df["pe_vs_sector_median_pct"].round(
    2
)
...
valuation_df["flag"] = valuation_df.apply(valuation_flag, axis=1)
...
valuation_summary.to_excel(summary_file, index=False)
...
valuation_flags.to_csv(flags_file, index=False)
