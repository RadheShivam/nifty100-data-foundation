...
def test_company_id_uppercase():
    df = pd.DataFrame({"company_id": ["reliance", "tcs", "infy"]})

    df["company_id"] = df["company_id"].str.upper()

    assert list(df["company_id"]) == ["RELIANCE", "TCS", "INFY"]
...
def test_string_trim():
    df = pd.DataFrame({"company_name": [" Reliance Industries ", " TCS "]})
...
def test_year_column_type():
    df = pd.DataFrame({"year": ["Mar 2024", "Mar 2023"]})
